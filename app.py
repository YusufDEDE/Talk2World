...
def translate_text():
    ...
    translated = GoogleTranslator(source='tr', target='en').translate(paragraph)
...
def enter_room(room_id):
    if room_id not in session:
        return redirect(url_for("entry_checkpoint", room_id=room_id))
    return render_template("chatroom.jinja2", room_id=room_id, display_name=session[room_id]["name"],
...
def entry_checkpoint(room_id):
    ...
    return render_template("chatroom_checkpoint.jinja2", room_id=room_id)
